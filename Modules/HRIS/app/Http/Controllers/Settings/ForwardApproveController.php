...
class ForwardApproveController extends Controller
{
    ...
    //             if ($accessId != 0 && $accessId != $request->org_id) {
    //                 return redirect()->back()->with('error', 'You are not allowed to give permission for this organization.');
    //             }
    ...
    public function store(Request $request)
    {
        ...
        $accessId = User::find($request->user_id)->access_id;
        if ($accessId != 0 && $accessId != $request->org_id) {
            return redirect()->back()->with('error', 'You are not allowed to give this user permissions for this organization.');
        }

        $table = $request->type == 1 ? 'hris_settings_employee_leave_forwardapprove' : 'hris_settings_employee_gatepass_approve';
