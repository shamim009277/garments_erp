            ...
            <div class="card">
                <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2 py-3"
                    ...
                    <div class="filter-select" style="max-width: 400px;">
                        <x-select-input style="max-width: 400px !important;" name="org_id" id="org_id"
                            class="select2 w-100" :options="$organizations" required />
                    </div>
                </div>
                ...
                <div class="card-body">
                    <div class="row row-cols-2 row-cols-md-4 row-cols-xl-8 g-3">
                        ...
                        <div class="col">
                            <div class="card border-0 shadow-sm text-white h-100"
                                style="background:linear-gradient(135deg,#ef6c00,#fb8c00);">
                                <div class="card-body">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div>
        ...
        <div class="col-md-4 pe-md-0" style="margin-bottom:10px;">
            <div class="card dashboard-chart-card h-100">
                <div class="card-header border-bottom d-flex align-items-center justify-content-between flex-wrap gap-2">
                    ...
                    <div class="text-end">
                        <span class="badge bg-primary">{{ number_format($totalChartEmployees) }} Total</span>
                        <div class="text-muted font-size-11 mt-1">{{ $totalChartCompanies }}
                ...
                <div class="card-body">
                    ...
                    {{-- @if ($companyWiseEmployees->isNotEmpty())
                        <div class="mt-3 pt-2 border-top">
                            @foreach ($companyWiseEmployees as $index => $company)
                                <div class="company-legend-item">
                                    <div class="d-flex align-items-center text-truncate me-2">
                                        <span class="legend-dot"
                                            style="background-color: {{ $chartColors[$index % count($chartColors)] }};"></span>
                                        <span class="font-size-13 text-truncate"
                                            title="{{ $company['name'] }}">{{ $company['short_name'] }}</span>
                                    </div>
                                    <span
                                        class="badge bg-light text-dark border">{{ number_format($company['total']) }}</span>
                                </div>
                            @endforeach
                        </div>
                    @endif --}}
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const companyData = @json($companyWiseEmployees);
            const chartColors = @json($chartColors);
            const totalEmployees = {{ $totalChartEmployees }};

            const labels = companyData.map(item => item.short_name);
            const series = companyData.map(item => item.total);

            const chartEl = document.querySelector('#company-employee-chart');
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const companyData = @json($companyWiseMovement);
            const chartColors = @json($chartColors);

            const categories = companyData.map(item => item.short_name);

            const lastJoining = companyData.map(item => item.last_month_joining);
            const currentJoining = companyData.map(item => item.current_month_joining);
            const lastResigned = companyData.map(item => item.last_month_resigned);
            const currentResigned = companyData.map(item => item.current_month_resigned);

            const chartEl = document.querySelector('#company-employee-movement-chart');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const companyData = @json($companyAttendance);
    ...
    const options = {
        ...
        series: [
            {
                name: 'Total Employee',
                data: companyData.map(item => item.total_employee)
            },
            {
                name: 'Punched',
                data: companyData.map(item => item.punched_employee)
            },
            {
                name: 'Not Punched',
                data: companyData.map(item => item.not_punched)
            }
        ],
        ...
        tooltip: {
            ...
            custom: function ({ dataPointIndex }) {
                ...
                const row = companyData[dataPointIndex];
    ...
    new ApexCharts(
        document.querySelector("#company-attendance-radar"),
        options
    ).render();
