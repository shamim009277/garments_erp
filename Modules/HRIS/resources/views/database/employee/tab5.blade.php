                ...
                <div class="card alert-primary alert-top-border padding-card">
                    ...
                    <div class="card-body">
                        <table id="datatable" class="table table-striped mb-0" width="100%">
                            ...
                            <tbody>
                                @foreach ($employee_experience as $key => $experience)
                                    <tr id="row-{{ $experience->id }}">
                                        ...
                                        <td style="text-align: center;">
                                            <a href="#" class="btn btn-soft-success waves-effect waves-light" style="padding: 4px 6px;" data-bs-toggle="modal" data-bs-target="#editModal{{ $experience->id }}"><i class="fas fa-edit"></i></a>
                                            <a href="#" class="btn btn-soft-danger waves-effect waves-light delete-training" data-id="{{ $experience->id }}" style="padding: 4px 6px;"><i class="fas fa-trash"></i></a>
                                        </td>
                                    </tr>
                                    <div id="editModal{{ $experience->id }}" class="modal fade" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true" data-bs-scroll="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <form id="editForm{{ $experience->id }}" action="{{ route('hris.database.employee-experience.update', $experience->id) }}" method="POST">
                                                    <div class="modal-body">
                                                        @csrf
                                                        @method('PUT')
                                                        <input type="hidden" name="employee_id" value="{{ $experience->employee_id }}">
                                                        <x-input-group name="designation" label="Designation" type="text" placeholder="Enter designation" :value="$experience->designation" required />
                                                        <x-input-group name="organization" label="Organization" type="text" placeholder="Enter organization" :value="$experience->organization" required />
                                                        <x-input-group name="description" label="Description" type="text" placeholder="Enter description" :value="$experience->description" />
                                                        <x-input-group name="duration" label="Duration" type="text" placeholder="Enter duration" :value="$experience->duration" required />
                                                    </div>
                    ...
                    <div class="card-body" style="padding:10px 10px;">
                        <table class="table table-striped mb-0" id="academicTable" width="100%">
                            ...
                            <tr>
                                ...
                                <td width="70%" style="border: none;"><x-text-input name="designation" id="designation" label="" class="form-control-sm" placeholder="Designation" required /></td>
                            </tr>
                            <tr>
    ...
    $(document).ready(function() {
        $(document).on('click', '.delete-experience', function(e) {
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route('hris.database.employee-experience.delete') }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: experienceId
