    ...
    <div class="row">
                                ...
                                <tr id="row-{{ $group->id }}">
                                    ...
                                    <td>
                                        ...
                                        <a href="#"
                                            ...
                                            data-id="{{ $group->id }}" style="padding: 4px 6px;"><i
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="editModalLabel{{ $group->id }}">Edit
                                                    Group</h5>
                                            ...
                                            <div class="modal-body">
                                                <form id="moduleForm"
                                                    action="{{ route('sms.setup.groups.update', $group->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')

                                                    <x-input-group name="name" label="Name" :value="$group->name"
                                                        required />
                                                    <x-input-group name="description" label="Description" :value="$group->description"
                                                        required />
        ...
        <div class="col-md-4">
            <div class="card alert-info alert-top-border">
                ...
                <div class="card-body">
                    <form id="moduleForm" action="{{ route('sms.setup.groups.store') }}" method="POST">
                        @csrf
    ...
    <script>
        $(document).ready(function() {
            $('.group-toggle').on('change', function() {
                ...
                $.ajax({
                    url: '{{ route('sms.setup.groups.toggle') }}',
                    type: 'POST',
                    data: {
                        id: id,
                        status: status,
        ...
        $(document).on('click', '.delete-group', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route('sms.setup.groups.destroy', ':id') }}'.replace(':id', id),
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}',
                        },
