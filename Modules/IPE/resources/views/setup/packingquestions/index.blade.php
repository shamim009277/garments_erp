    ...
    <div class="row">
        <div class="col-12">
            @include('components.breadcrumb', [
                'title' => 'IPE',
                'subtitle' => 'Packing Questions',
                'breadcrumbs' => [
                    ['label' => 'IPE', 'url' => route('ipe.index')],
                    ['label' => 'Setup', 'url' => route('ipe.index')],
                    ['label' => 'Packing Questions', 'url' => route('ipe.setup.packingquestions.index')],
                ],
            ])
        </div>
        <div class="col-lg-12">
            <div class="card alert-info alert-top-border">
                <div class="card-header" style="padding: 12px 10px !important">
                    <h6 class="my-0 text-primary"> <i class="mdi mdi-list"></i> Packing Questions List</h6>
                </div>
                <form id="moduleForm" action="{{ route('ipe.setup.packingquestions.store') }}" method="POST">
                    @csrf
                    <div class="card-body">
                                    ...
                                    <tr id="row-{{ $packingQuestion->id }}">
                                        ...
                                        <td>
                                            <div class="square-switch">
                                                <input type="checkbox" id="square-switch3{{ $packingQuestion->id }}" class="packing-question-toggle" data-id="{{ $packingQuestion->id }}" switch="bool" {{ $packingQuestion->is_active ? 'checked' : '' }} />
                                        ...
                                        <td class="text-center">
                                            ...
                                            <a href="#" class="btn btn-soft-danger waves-effect waves-light delete-packing-question" data-id="{{ $packingQuestion->id }}" style="padding: 4px 6px;"><i class="fas fa-trash"></i></a>
                                        </td>
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h6 class="modal-title" id="myModalLabel">Edit Packing Question</h6>
                                                        <button type="button" class="btn-close btn btn-sm" data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>

                                                    <form id="editForm{{ $packingQuestion->id }}" action="{{ route('ipe.setup.packingquestions.update', $packingQuestion->id) }}" method="POST">
                                                        <div class="modal-body">
                                                            @csrf
                                                            @method('PUT')
                                                            <x-input-group name="sl" label="SL" type="text" placeholder="Enter SL" :value="$packingQuestion->sl" required />
...
@push('scripts')
    <script>
        $(document).ready(function() {
            $('.packing-question-toggle').on('change', function() {
                let id = $(this).data('id');
                let status = $(this).is(':checked') ? 1 : 0;
                $.ajax({
                    url: '{{ route('ipe.setup.packingquestions.toggle') }}',
                    type: 'POST',
                    data: {
                        id: id,
                        status: status,
        ...
        $(document).on('click', '.delete-packing-question', function(e) {
            e.preventDefault();
            let packingQuestionId = $(this).data('id');
            Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route('ipe.setup.packingquestions.delete') }}',
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            id: packingQuestionId
                        },
                        success: function(response) {
                            Swal.fire(
                                'Deleted!',
