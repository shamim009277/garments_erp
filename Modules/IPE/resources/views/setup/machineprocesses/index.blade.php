        ...
        <div class="col-12">
            @include('components.breadcrumb', [
                ...
                'breadcrumbs' => [
                    ...
                    ['label' => 'Process List', 'url' => route('ipe.setup.machineprocesses.index')],
                ],
            ])
        </div>
        <div class="col-lg-12">
            <div class="card alert-info alert-top-border">
                ...
                <form id="moduleForm" action="{{ route('ipe.setup.machineprocesses.store') }}" method="POST">
                    @csrf
                    <div class="card-body">
                        <table id="datatable" class="table table-bordered dt-responsive  nowrap w-100" width="100%">
                            <thead>
                                <tr>
                                    <th width="2%">SL</th>
                                    <th width="5%">Machine Type</th>
                                    <th width="7%">Process</th>
                                    <th width="8%">Code</th>
                                    <th width="12%">Item</th>
                                    <th width="10">Process Name</th>
                                    <th width="10">Process Bangla</th>
                            ...
                            <tbody>
                                <tr>
                                    <td>1</td>
                                    <td>
                                        <select name="type_id" id="type_id" class="form-control form-control-sm @error('type_id') is-invalid @enderror"required>
                                    ...
                                    <td>
                                         <input type="text" name="process" class="form-control form-control-sm @error('process') is-invalid @enderror" placeholder="i.e. Process" required >
                                        @error('process')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </td>
                                    ...
                                    <td>
                                        <select name="process_type" id="process_type" class="form-control form-control-sm @error('process_type') is-invalid @enderror" required>
                                            <option value="1">Basic</option>
                                            <option value="2">Semi Critical</option>
                    ...
                    <tr id="row-{{ $process->id }}">
                        ...
                        <td>
                            <span class="badge {{ $process->process_type == 1 ? 'bg-primary' : 'bg-success' }}">
                                @if($process->process_type == 1)
                                   Basic
                                @elseif($process->process_type == 2)
                                    Semi Critical
                                @elseif($process->process_type == 3)
                                    Critical
                                @endif
                            </span>
                        </td>
                        ...
                        <td class="text-center">
                            <div class="square-switch">
                                <input type="checkbox" id="square-switch3{{ $process->id }}" class="process-toggle" data-id="{{ $process->id }}" switch="bool" {{ $process->is_active ? 'checked' : '' }} />
                                <label for="square-switch3{{ $process->id }}" data-on-label="Yes" data-off-label="No" style="margin: 0px; vertical-align: middle;"></label>
                            </div>
                        </td>
                        <td class="text-center">
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h6 class="modal-title" id="myModalLabel">Edit Machine Process</h6>
                                        <button type="button" class="btn-close btn btn-sm" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>

                                    <form id="editForm{{ $process->id }}" action="{{ route('ipe.setup.machineprocesses.update', $process->id) }}" method="POST">
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <select name="type_id" id="type_id" class="form-control form-control-sm @error('type_id') is-invalid @enderror"required>
                                                    @foreach ($machines as $machine)
                                                        <option value="{{ $machine->id }}" {{ $process->type_id == $machine->id ? 'selected' : '' }}>{{ $machine->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="mb-2">
                                                <label>Process</label>
                                                <input type="text" name="process" value="{{ $process->process }}" class="form-control form-control-sm @error('process') is-invalid @enderror" placeholder="i.e. Process" required >
                                            </div>
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <select name="process_type" id="process_type" class="form-control form-control-sm @error('process_type') is-invalid @enderror" required>
                                                    <option value="1" {{ $process->process_type == 1 ? 'selected' : '' }}>Basic</option>
                                                    <option value="2" {{ $process->process_type == 2 ? 'selected' : '' }}>Semi Critical</option>
                                                    <option value="3" {{ $process->process_type == 3 ? 'selected' : '' }}>Critical</option>
                                                </select>
                                            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('.process-toggle').on('change', function() {
                ...
                $.ajax({
                    url: '{{ route('ipe.setup.machineprocesses.toggle') }}',
                    type: 'POST',
                    data: {
                        id: id,
                        status: status,
        ...
        $(document).on('click', '.delete-process', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route('ipe.setup.machineprocesses.delete') }}',
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            id: processId
