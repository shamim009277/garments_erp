...
<body>
    ...
    <header>
        <div style="display: flex; align-items: center; justify-content: center; gap: 10px;">
            <img src="{{ public_path('backend/assets/images/logo-sm.svg') }}" alt="Logo" style="width: 35px; height: 35px;">
            <div class="company-info">
                <div style="font-weight: bold; font-size: 13px;">{{ $general->full_name }}</div>
        ...
        <table class="table table-bordered table-hover table-striped mb-0">
            ...
            <tbody>
                @if($assessment->details && count($assessment->details) > 0)
                    @php
                        $efficiency = ($assessment->details->where('status', 1)->count() / count($assessment->details)) * 100;
                    @endphp
                    <tr>
                        <td>General Question</td>
                        <td>N/A</td>
                        <td>{{ count($assessment->details)*5 }}</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>{{ $assessment->details->where('status', 1)->count() * 5 }}</td>
                        <td>N/A</td>
                        <td>N/A</td>
                ...
                @foreach($assessment->processes as $process)
                    <tr>
                        <td>{{ $process->processName->process_name ?? 'N/A' }}</td>
                        <td>{{ $process->mc }}</td>
            ...
            @php
                $avgeff = ($assessment->processes->avg('efficiency') + $efficiency) / 2;
            @endphp
