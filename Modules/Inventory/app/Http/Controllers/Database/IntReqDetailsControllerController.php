...
use Modules\Inventory\Http\Requests\Database\IntReqDetailsControllerRequest;
...
class IntReqDetailsControllerController extends Controller
{
    /**
     ...
     */
    public function store(IntReqDetailsControllerRequest $request) {
        $data = $request->validated();
        $time= Carbon::now()->format('Y-m-d');
        $year = date('Y', strtotime($time));
        $yr = date('y', strtotime($time)); 
        $mn = date('m', strtotime($time)); 
        $req_id =IntReqMain::where('requisition_no','=',$data['req_no'])->first(); 
        $item = IntReqDetails::where('pur_req_id', $req_id->id)->where('item_id', $data['item_id'])->first();
        ...
        DB::beginTransaction();
        try {
            $reqdetail = IntReqDetails::create($data);
            DB::commit();
    ...
    public function update(Request $request, $id) {
        $reqdetail = IntReqDetails::find($id);
