...
class InternalReqControllerController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $purrequisitions = IntReqMain::where('is_done',0)->orderBy('id', 'desc')->take(50)->get();
        $items = Item::where('is_active',1)->get();
        $today_date = date('Y-m-d');
        return view('inventory::database.intreq.index', compact('today_date','organizations','store_locations','purrequisitions','items'));
    }
    ...
    public function pdfData($id){
        ...
        $purrequisition = IntReqMain::with('organization', 'required_by', 'store')->find($id);
