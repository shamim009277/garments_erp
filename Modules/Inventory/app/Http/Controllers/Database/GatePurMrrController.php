...
class GatePurMrrController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $gatepurmrrs = GatePurMrrMain::where('is_done',0)->orderBy('id', 'desc')->get();
        $items = Item::where('is_active',1)->get();
        $today_date = date('Y-m-d');
        $users = User::all();
        $suppliers = Supplier::all();
        return view('inventory::database.gatepurmrr.index', compact('today_date','organizations','store_locations','gatepurmrrs','items','users','suppliers'));
    }
