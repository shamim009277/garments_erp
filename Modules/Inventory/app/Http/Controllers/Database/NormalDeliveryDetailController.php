...
use Modules\Inventory\Http\Requests\Database\NormalDeliveryDetailRequest;
...
class NormalDeliveryDetailController extends Controller
{
    /**
     ...
     */
    public function store(NormalDeliveryDetailRequest $request) {
        $data = $request->validated();
        $time= Carbon::now()->format('Y-m-d');
        $year = date('Y', strtotime($time));
        $yr = date('y', strtotime($time)); 
        $mn = date('m', strtotime($time)); 
        $req_id =NormalDeliveryMain::where('requisition_no','=',$data['req_no'])->first(); 
        $item = NormalDeliveryDetail::where('pur_req_id', $req_id->id)->where('item_id', $data['item_id'])->first();
        
        if($req_id->is_done == 1){
            return response()->json(['success' => false, 'message' => 'Purchase Requisition is already done']);
        }else if($item){
        ...
        DB::beginTransaction();
        try {
            $reqdetail = NormalDeliveryDetail::create($data);
            DB::commit();
    ...
    public function update(Request $request, $id) {
        $reqdetail = NormalDeliveryDetail::find($id);
