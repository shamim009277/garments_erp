...
class ReadyToPurCompleteController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $purrequisitions = PurRequisitionMain::orderBy('id', 'desc')->where('purchase_stage',2)->get();
        $items = Item::where('is_active',1)->get();
        $today_date = date('Y-m-d');
        return view('inventory::database.purreqcomplete.index', compact('today_date','purrequisitions','items'));
    }
    ...
    public function search(Request $request) {
        ...
        $purrequisitions = PurRequisitionMain::where('requisition_no', 'like', '%'.$search.'%')->where('purchase_stage',2)->orderBy('id', 'desc')->take(10)->get();
    ...
    public function pdfData($id){
        ...
        $purrequisition = PurRequisitionMain::with('organization', 'required_by', 'store','done_by','forward_by','priced_by','confirmed_by','approved_by','rejected_by','fapproved_by')->find($id);
        $reqDetails = PurRequisitionDetail::with('item','pur_unit')->where('is_rejected',2)->where('pur_req_id', $id)->get();
        $title = 'Purchase Requisition';
        $pdf = Pdf::loadView('inventory::database.purreqcomplete.pdf', compact('purrequisition','reqDetails','title'))
