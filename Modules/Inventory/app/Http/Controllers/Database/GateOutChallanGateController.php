...
class GateOutChallanGateController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $gateoutchallans = GateOutChallanMain::where('is_done',1)->where('is_approved',1)->where('is_gate_out',0)->orderBy('id', 'desc')->take(50)->get();
        $challanpurposes = ChallanPurpose::where('is_active',1)->get();
        $suppliers = Supplier::where('is_active',1)->get();
        $items = Item::where('is_active',1)->get();
        $today_date = date('Y-m-d');
        return view('inventory::database.gateoutchallanagate.index', compact('today_date','organizations','store_locations','items','challanpurposes','gateoutchallans','suppliers'));
    }
    ...
    public function search(Request $request) {
        ...
        $match = $request->match;
        if($match == 3) {
            $purrequisitions = GateOutChallanMain::where('challan_no', 'like', '%'.$search.'%')->where('is_done',1)->where('is_approved',1)->orderBy('id', 'desc')->take(10)->get();
        } else if($match == 2) {
            $purrequisitions = GateOutChallanMain::where('challan_no', 'like', '%'.$search.'%')->where('is_done',1)->where('is_approved',1)->where('is_gate_out',1)->orderBy('id', 'desc')->take(50)->get();
        } else if($match == 1) {
            $purrequisitions = GateOutChallanMain::where('challan_no', 'like', '%'.$search.'%')->where('is_done',1)->where('is_approved',1)->where('is_gate_out',0)->orderBy('id', 'desc')->take(50)->get();
        }
