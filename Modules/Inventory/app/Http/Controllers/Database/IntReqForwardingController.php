...
class IntReqForwardingController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $purrequisitions = IntReqMain::where('is_done',1)->where('is_forward',0)->orderBy('id', 'desc')->get();
        $items = Item::where('is_active',1)->get();
        $today_date = date('Y-m-d');
        return view('inventory::database.intreqforwarding.index', compact('today_date','organizations','store_locations','purrequisitions','items'));
    }
    ...
    public function multipleStatus(Request $request, $id) {
        ...
        try {
            ...
            IntReqDetails::where('pur_req_id', $id)->update([
    ...
    public function search(Request $request) {
        ...
        if($match == 3) {
            $purrequisitions = IntReqMain::where('requisition_no', 'like', '%'.$search.'%')->where('is_done', 1)->orderBy('id', 'desc')->take(10)->get();
        } else if($match == 2) {
            $purrequisitions = IntReqMain::where('requisition_no', 'like', '%'.$search.'%')->where('is_forward', 1)->where('is_done', 1)->orderBy('id', 'desc')->take(50)->get();
        } else if($match == 1) {
            $purrequisitions = IntReqMain::where('requisition_no', 'like', '%'.$search.'%')->where('is_forward', 0)->where('is_done', 1)->orderBy('id', 'desc')->take(50)->get();
        }
