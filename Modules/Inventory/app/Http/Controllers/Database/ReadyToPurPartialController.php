...
class ReadyToPurPartialController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $purrequisitions = PurRequisitionMain::orderBy('id', 'desc')->where('purchase_stage',1)->get();
        $items = Item::where('is_active',1)->get();
        $today_date = date('Y-m-d');
        return view('inventory::database.purreqpartial.index', compact('today_date','purrequisitions','items'));
    }
    ...
    public function search(Request $request) {
        ...
        $purrequisitions = PurRequisitionMain::where('requisition_no', 'like', '%'.$search.'%')->where('purchase_stage',1)->orderBy('id', 'desc')->take(10)->get();
    ...
    public function pdfData($id){
        ...
        $reqDetails = PurRequisitionDetail::with('item','pur_unit')->where('is_rejected','!=',1)->where('pur_req_id', $id)->get();
        $title = 'Purchase Requisition';
        $pdf = Pdf::loadView('inventory::database.purreqpartial.pdf', compact('purrequisition','reqDetails','title'))
