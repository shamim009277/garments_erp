...
class GateOutChallanController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $gateoutchallans = GateOutChallanMain::where('is_done',0)->orderBy('id', 'desc')->take(50)->get();
        $challanpurposes = ChallanPurpose::where('is_active',1)->get();
        $suppliers = Supplier::where('is_active',1)->get();
        $items = Item::where('is_active',1)->get();
        $today_date = date('Y-m-d');
        return view('inventory::database.gateoutchallan.index', compact('today_date','organizations','store_locations','purrequisitions','items','challanpurposes','gateoutchallans','suppliers'));
    }
