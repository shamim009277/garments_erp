...
class PurchaseAuditController extends Controller
{
    ...
    public function index()
    {
        ...
        $gatepurmrrs = GatePurMrrMain::where('is_store_rcv',1)->orderBy('id', 'desc')->get();
        $items = Item::where('is_active',1)->get();
        $today_date = date('Y-m-d');
        $users = User::all();
        $suppliers = Supplier::all();
        return view('inventory::database.puraudit.index', compact('today_date','organizations','store_locations','purrequisitions','gatepurmrrs','items','users','suppliers'));
    }
