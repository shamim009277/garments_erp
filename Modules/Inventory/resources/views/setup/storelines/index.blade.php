...
@section('content')
    <div class="row">
        ...
        // $table->boolean('is_active')->default(true); --}}
                        ...
                        <tbody>
                            @foreach ($storelines as $key => $storeline)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $storeline->name }}</td>
                                    <td>{{ $storeline->description }}</td>
                                    <td class="text-center">
                                        <p class="text-{{ $storeline->is_active ? 'success' : 'danger' }}">
                                            {{ $storeline->is_active ? 'Active' : 'Inactive' }}</p>
                                    </td>
                                    <td>
                                        <a href="#" class="btn btn-soft-success waves-effect waves-light"
                                            style="padding: 4px 6px;" data-bs-toggle="modal"
                                            data-bs-target="#editModal{{ $storeline->id }}"><i class="fas fa-edit"></i></a>
                                        <form action="{{ route('inventory.setup.storelines.destroy', $storeline->id) }}"
                                ...
                                {{-- load edit modal --}}
                                <div class="modal fade" id="editModal{{ $storeline->id }}" tabindex="-1"
                                    aria-labelledby="editModalLabel{{ $storeline->id }}" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="editModalLabel{{ $storeline->id }}">Edit Store
                                                    Line</h5>
                                            ...
                                            <div class="modal-body">
                                                <form id="moduleForm"
                                                    action="{{ route('inventory.setup.storelines.update', $storeline->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')

                                                    <x-input-group name="name" label="Name" :value="$storeline->name"
                                                        required />
                                                    <x-input-group name="description" label="Description"
                                                        :value="$storeline->description" />
                                                    <x-select-input-group name="is_active" label="Is Active?"
                                                        :options="['1' => 'Active', '0' => 'Inactive']" :selected="$storeline->is_active ? '1' : '0'" required />
                                                    <x-primary-button
        ...
        <div class="col-md-4">
            <div class="card alert-info alert-top-border">
                ...
                <div class="card-body">
                    <form id="moduleForm" action="{{ route('inventory.setup.storelines.store') }}" method="POST">
                        @csrf
