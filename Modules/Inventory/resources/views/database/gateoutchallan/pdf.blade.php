...
<body>
    ...
    <table class="no-border">
        <caption style="text-align: center;text-transform: uppercase;"><h4><span style="border: 1px solid black; padding: 5px;">{{ $title }}</span></h4></caption>
        <tr><th>Challan No</th><td>:{{ $challanMain->challan_no }}</td><th >Organization</th><td>:{{ $challanMain->organization->name }}</td></tr>
        <tr><th>Challan Date</th><td>:{{ $challanMain->challan_date }}</td><th >Store</th><td>:{{ $challanMain->store->name }}</td></tr>
        <tr><th>Challan By</th><td>:{{ $challanMain->challan_by->name }}</td><th >Note</th><td>:{{ $challanMain->note }}</td></tr>
        <tr><th>Purpose of Challan</th><td>:{{ $challanMain->purpose->purpose_name }}</td><th >Printed by</th><td>:{{ auth()->user()->name ?? 'System' }}<br>{{ now()->format('d-m-Y h:i A') }}</td></tr>
    </table>
    ...
    <table> 
        ...
        @foreach ($challanDetails as $x)
            <tr>
                <td style="text-align: center;">{{ $loop->iteration }}</td>
                <td style="text-align: center;">{{ @$x->item->item_name }}</td>
                <td style="text-align: center;">{{ @$x->challan_qty }}</td>
                <td style="text-align: center;">{{ @$x->unit->name }}</td>
