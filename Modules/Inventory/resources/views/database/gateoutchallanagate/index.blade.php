    ...
    <script>
        $(document).ready(function() {
            ...
            function getChallanMain(id){
                $.ajax({
                    type: 'GET',
                    url: "{{ url('inventory/database/gateoutchallangate') }}/"+id,
            ...
            $("#done-btn").off('click').on('click',function() {
                ...
                $.ajax({
                    ...
                    data: {
                        is_gate_out: 1,
                        gate_out_by_id: "{{ Auth::user()->id }}",
                        gate_out_date: "{{ date('Y-m-d') }}",
                        _token: "{{ csrf_token() }}"
                    },
                    success: function(data) {
                        console.log(data);
            ...
            function searchRequisition(match, search) {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "{{ route('inventory.database.gateoutchallangate.search') }}",
                    data: {
                        search: search,
                        match: match,
                        _token: "{{ csrf_token() }}"
