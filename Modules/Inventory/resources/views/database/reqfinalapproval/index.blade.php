    ...
    <script>
        $(document).ready(function() {
            ...
            $("#done-btn").click(function() {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "{{ url('inventory/database/reqfinalapproval/multiplestatus') }}/"+req_id,
                    data: {
                        is_fapproved: 1,
                        _token: "{{ csrf_token() }}"
                    },
                        ...
                        $.each(detailsData, function(key, value) {
                            ...
                            if(value.is_rejected == 1 && value.rejected_stage != 3){
                                ...
                                <tr>
                                    ...
                                    <td><input type="text" class="form-control form-control-sm fupdate-item" data-id="${value.id}" id="for_qty_${value.id}" value="${value.for_qty?value.for_qty:0}"></td>
            ...
            function searchRequisition(match, search) {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "{{ route('inventory.database.reqfinalapproval.search') }}",
                    data: {
                        search: search,
                        match: match,
                        _token: "{{ csrf_token() }}"
