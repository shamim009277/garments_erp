<div class="row g-3">
    <div class="col-lg-12">
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body">
                <form method="POST"
                    action="{{ route('inventory.database.basicorders.colors_sizes.store', $basicorder->id) }}">
                    @csrf
                    <div class="row g-3">
                        <div class="col-md-2">
                    ...
                    <div class="row g-3">
                        ...
                        @foreach ($lots as $lot)
                            ...
                            <div class="col-md-12">
                                <div class="mb-3">
                                    <label class="form-label">Color</label>
                                    <select class="form-select" name="lots[{{ $loop->index }}][color_id]">
                            ...
                            <div class="col-md-12">
                                <div class="mb-3">
                                    <label class="form-label">Size Group</label>
                                    <select class="form-select" name="lots[{{ $loop->index }}][size_group_id]">
                                        @foreach ($sizeGroups as $sizeGroup)
                                            <option value="{{ $sizeGroup->id }}">{{ $sizeGroup->size_group_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" name="lots[{{ $loop->index }}][size_id]">
                                        @foreach ($sizes as $size)
                                            <option value="{{ $size->id }}">{{ $size->size_name }} with color QTY: {{ $size->size_quantity }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" name="lots[{{ $loop->index }}][quantity]" value="{{ $lot->lot_quantity }}" >
                                </div>
                            </div>
                            @endforeach
