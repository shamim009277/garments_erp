    ...
    <div class="row">
        <div class="col-12">
            @include('components.breadcrumb', [
                'title' => 'INVENTORY',
                'subtitle' => 'Purchase Account Bill',
                'breadcrumbs' => [
                    ['label' => 'INVENTORY', 'url' => route('inventory.index')],
                    ['label' => 'Database', 'url' => route('inventory.index')],
                    ['label' => 'Purchase Audit', 'url' => route('inventory.database.puracc.index')],
                ],
            ])
        </div>
                            ...
                            <table class="table table-bordered dt-responsive  nowrap w-100 text-center p-2" width="100%">
                                <thead>
                                    <tr class="p-0">
                                        <th width="5%">#SL</th>
                                        <th width="20%">Name</th>
                                        <th width="10%">Gate Receive Qty</th>
                                        <th width="10%">Store Receive Qty</th>
                                        <th width="10%">Unit</th>
                            ...
                            <table class="table table-bordered dt-responsive  nowrap w-100 text-center p-2" width="100%">
                                ...
                                <tbody>
                                      <tr class="p-0">
                                       ...
                                       <td width="10%">
                                            <input type="text" class="form-control form-control-sm calculate_bill" id="bill_amount" name="" value="0" readonly>
                                        </td>
                                       <th width="10%">Paid Amount</th>
                                       <td width="10%">
                                            <input type="text" class="form-control form-control-sm calculate_bill" id="paid_amount" name="" value="0">
                                        </td>
                                       <th width="10%">Due Amount</th>
                                       <td width="10%">
                                            <input type="text" class="form-control form-control-sm calculate_bill" id="due_amount" name="" value="0">
                                        </td>
                                       <th width="10%">                            
                                            <button type="button" class="btn btn-sm btn-success d-none" id="payBtn">Paid</button>
                                            <button type="button" class="btn btn-sm btn-warning d-none" id="unpaidBtn">Unpaid</button>
                                        </th>
                                    </tr>
                                </tbody>
                            </table>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            $('.calculate_bill').on('keyup', function() {
                Calculatebill();
            });

            $('.calculate_bill').on('change', function() {
                Calculatebill();
            });
            ...
            function Calculatebill(){
                ...
                $('#due_amount').val(Number(bill_amount) - Number(paid_amount));
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#mrr_no-txt').text(mainData.mrr_no);
                            ...
                            $('#bill_amount').val(mainData.bill_amount);
                            $('#paid_amount').val(mainData.paid_amount);
                            $('#due_amount').val(mainData.due_amount);
                            ...
                            $('#audit_iframe').attr('src', mainData.document);
                            $('#search_data').val('').trigger('change');
                            if(mainData.is_bill_paid == 1){
                        ...
                        $.each(detailsData, function(key, x) {
                            ...
                            $('#detail-item-row').append(`
                                <tr class="p-0" id="detail-item-row-${x.id}">
                                    ...
                                    <td width="10%">${x.store_rcv_qty}</td>
            ...
            $("#payBtn").off('click').on('click',function() {
                ...
                $.ajax({
                    ...
                    data: {
                        is_bill_paid: 1,
                        bill_paid_by: "{{ Auth::user()->id }}",
                        bill_paid_date: "{{ $today_date }}",
            ...
            $("#unpaidBtn").off('click').on('click',function() {
                let req_id = $('#u_mrr_id').val();
                let bill_amount = $('#bill_amount').val();
                $.ajax({
                    type: 'PUT',
