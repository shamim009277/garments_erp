    ...
    <div class="row">
        <div class="col-12">
            @include('components.breadcrumb', [
                'title' => 'INVENTORY',
                'subtitle' => 'Store Receive Purchase Requisition',
                'breadcrumbs' => [
        ...
        <div class="col-12 mb-3">
            <div class="d-flex flex-column flex-md-row align-items-center justify-content-between">
                ...
                <h4 class="text-center flex-grow-1 order-1 order-md-0 mb-2 mb-md-0">
                    Store Receive Purchase Requisition
                </h4>
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateItem(id){
                let store_rcv_qty = $('#store_rcv_qty_'+id).val();
                let pur_price = $('#pur_price_'+id).val();
                $.ajax({
                    type: 'PUT',
                    url: "{{ url('inventory/database/purreqstorercv') }}/"+id,
                    data: {
                        store_rcv_qty: store_rcv_qty,
                        pur_price: pur_price,
                        is_store_received: 1,
                        store_rcv_by: "{{ Auth::user()->id }}",
                        store_rcv_date: "{{ $today_date }}",
                        _token: "{{ csrf_token() }}"
                    },
            ...
            function removeItem(id){
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        pass_qty: pass_qty,
                        is_qa_pass: 0,
                        qa_check_by: "{{ Auth::user()->id }}",
                        qa_check_date: "{{ $today_date }}",
                        note: note,
                        _token: "{{ csrf_token() }}"
                    },
                    success: function(data) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(detailsData, function(key, x) {
                            let btnElement = mainData.is_store_rcv == 1 ? 'N\\A' : `<button type="button" data-id="${x.id}"   class="btn btn-info btn-sm p-1 update-item" id="update-item-${x.id}">Update</button>`;
                            if(x.is_qa_pass == 1){
                                ...
                                <tr class="p-0" id="detail-item-row-${x.id}">
                                    ...
                                    <td width="10%">
                                        <input type="text" class="form-control form-control-sm call-pur-amount" data-id="${x.id}" id="store_rcv_qty_${x.id}" name="" value="${x.store_rcv_qty == null ? x.received_qty : x.store_rcv_qty}">
                                    </td>
                                    <td width="10%">
                                        <input type="text" class="form-control form-control-sm call-pur-amount" data-id="${x.id}" id="pur_price_${x.id}" name="" value="${x.pur_price ?? 0}">
                                    </td>
                                    <td width="10%">
                                        <input type="text" class="form-control form-control-sm" data-id="${x.id}" id="pur_amount_${x.id}" name="" value="${x.pur_amount ?? 0}" readonly>
                                    </td>
                                    <td width="10%">
                                       ${btnElement}
                                    </td>
                        ...
                        $('.call-pur-amount').off('keyup').on('keyup', function() {
                            let id = $(this).data('id');
                            let store_rcv_qty = $('#store_rcv_qty_'+id).val();
                            let pur_price = $('#pur_price_'+id).val();
                            let pur_amount = Number(store_rcv_qty * pur_price).toFixed(2);
                            $('#pur_amount_'+id).val(pur_amount);
                        });

                        $('.call-pur-amount').off('change').on('change', function() {
                            let id = $(this).data('id');
                            let store_rcv_qty = $('#store_rcv_qty_'+id).val();
                            let pur_price = $('#pur_price_'+id).val();
                            let pur_amount = Number(store_rcv_qty * pur_price).toFixed(2);
            ...
            function searchGatePurMrr(match, search) {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "{{ route('inventory.database.purreqstorercv.search') }}",
                    data: {
                        search: search,
                        match: match,
                        _token: "{{ csrf_token() }}"
