    ...
    <div class="row">
        <div class="col-12">
            @include('components.breadcrumb', [
                'title' => 'INVENTORY',
                'subtitle' => 'Purchase Audit',
                'breadcrumbs' => [
                    ['label' => 'INVENTORY', 'url' => route('inventory.index')],
                    ['label' => 'Database', 'url' => route('inventory.index')],
                    ['label' => 'Purchase Audit', 'url' => route('inventory.database.puraudit.index')],
                ],
            ])
        </div>
                    ...
                    <div class="col-md-8">
                        <div class="card alert-info alert-top-border padding-card">
                            ...
                            <div class="card-footer">
                                <button type="button" class="btn btn-sm btn-primary d-none" id="pendingBtn">Pending</button>
                                <button type="button" class="btn btn-sm btn-secondary d-none" id="observeBtn">Observe</button>
                                <button type="button" class="btn btn-sm btn-success d-none" id="okayBtn">Okay</button>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <iframe id="audit_iframe" src=""  height="600px" width="100%"></iframe>
                    </div>
                </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#mrr_no-txt').text(mainData.mrr_no);
                            ...
                            if(mainData.is_audit_chck==0){
                                $('#pendingBtn').addClass('d-none');
                                $('#observeBtn').removeClass('d-none');
                                $('#okayBtn').removeClass('d-none');
                            }else if(mainData.is_audit_chck==1){
                                 $('#pendingBtn').removeClass('d-none');
                                $('#observeBtn').addClass('d-none');
                                $('#okayBtn').removeClass('d-none');
                            }else if(mainData.is_audit_chck==2){
                                $('#pendingBtn').removeClass('d-none');
                                $('#observeBtn').removeClass('d-none');
                                $('#okayBtn').addClass('d-none');
                            }
            ...
            $("#pendingBtn").off('click').on('click',function() {
             ...
             $("#observeBtn").off('click').on('click',function() {
                let req_id = $('#u_mrr_id').val();
                $.ajax({
                    type: 'PUT',
                    url: "{{ url('inventory/database/gatepurmrrmains/multiplestatus') }}/"+req_id,
