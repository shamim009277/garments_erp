    ...
    <script>
        $(document).ready(function() {
            ...
            $("#done-btn").click(function() {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "{{ url('inventory/database/reqpricing/multiplestatus') }}/"+req_id,
                    data: {
                        is_priced: 1,
                        _token: "{{ csrf_token() }}"
                    },
                        ...
                        $.each(detailsData, function(key, value) {
                            ...
                            if(value.is_rejected == 1 && value.rejected_stage == 1){
                                ...
                                <tr>
                                    ...
                                    <td width="10%">
                                        <input type="text" class="form-control form-control-sm approx_priced" data-qty="${value.for_qty}" data-id="${value.id}" id="approx_priced_${value.id}" value="${value.aprx_priced?value.aprx_priced:0}" ${(mainData.is_priced == 1 || mainData.is_rejected == 1) ? 'disabled' : ''}>
                                    </td>
                                    <td id="total_value_${value.id}">${value.total_value?value.total_value:0}</td>
                                    <td>
                                        ${mainData.is_priced == 1 ? 'N/A' : `<button type="button" data-id="${value.id}" class="btn btn-info btn-sm p-1 update-item">Save</button>`}
                                    </td>
                                </tr>
                            `);
                            }
            ...
            function searchRequisition(match, search) {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "{{ route('inventory.database.reqpricing.search') }}",
                    data: {
                        search: search,
                        match: match,
                        _token: "{{ csrf_token() }}"
