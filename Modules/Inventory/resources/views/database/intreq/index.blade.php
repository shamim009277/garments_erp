                                    ...
                                    <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
                                        <div class="modal-dialog modal-xl">
                                            <div class="modal-content">
                                                ...
                                                <form id="addFormMain" action="{{ route('inventory.database.intrequisitionmains.store') }}" method="POST">
    ...
    <script>
        $(document).ready(function() {
            ...
            function getReqMain(id){
                $.ajax({
                    type: 'GET',
                    url: "{{ url('inventory/database/intrequisitionmains') }}/"+id,
            ...
            $('#addFormMain').submit(function(e) {
                ...
                $.ajax({
                    url: '{{ route('inventory.database.intrequisitionmains.store') }}',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
            ...
            $("#done-btn").off('click').on('click',function() {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "{{ url('inventory/database/intrequisitionmains/multiplestatus') }}/"+req_id,
                    data: {
                        is_done: 1,
            ...
            $(document).on('click', '#deleteBtn', function(e) {
                ...
                if(purMainId == null || purMainId == undefined || purMainId == ''){
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ url('inventory/database/intrequisitionmains') }}/'+purMainId,
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}',
                                id: purMainId
            ...
            function searchRequisition(match, search) {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "{{ route('inventory.database.intrequisitionmains.search') }}",
                    data: {
                        search: search,
                        match: match,
                        _token: "{{ csrf_token() }}"
            ...
            $('#search_data').on('select2:select', function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('inventory.database.intrequisitiondetails.store') }}",
                    data: {
                        item_id: itemId,
                        req_no: req_no,
                        _token: "{{ csrf_token() }}"
            ...
            function updateItem(id){
                ...
                $.ajax({
                    type: 'PUT',
                    url: "{{ url('inventory/database/intrequisitiondetails') }}/"+id,
                    data: {
                        req_qty: reqQty,
                        for_qty: reqQty,
                        app_qty: reqQty,
