...
class BasicOrderController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('ordermanagement::database.basicorders.index', compact('basicorders', 'organizations', 'buyers', 'product_categories', 'merchandisers', 'fabric_types', 'compositions', 'fabric_treatments', 'yarn_counts', 'yarn_categories', 'ListOfOrdersUniqueBuyer', 'ListOfOrders'));
    }
    ...
    public function show(Request $request, $id)
    {
        ...
        if ($tab == 1) {
            ...
            $ListOfOrdersUniqueBuyer = $ListOfOrders->unique('buyer_id');
            $basicorder = BasicOrder::findOrFail($id);
            return view('ordermanagement::database.basicorders.show', compact('basicorder', 'basicorders', 'buyers', 'organizations', 'product_categories', 'merchandisers', 'fabric_types', 'compositions', 'fabric_treatments', 'yarn_counts', 'yarn_categories', 'ListOfOrdersUniqueBuyer', 'ListOfOrders', 'tab', 'lots', 'colors', 'sizes'));
        } elseif ($tab == 3) {
            $basicorders = BasicOrder::all();
            $buyers = Buyer::all();
            $organizations = Organization::all();
            ...
            $ListOfOrdersUniqueBuyer = $ListOfOrders->unique('buyer_id');
            $basicorder = BasicOrder::findOrFail($id);
            $lotColorsSizes = OrderLotColorSize::where('inventory_setup_order_lot_color_sizes.order_id', $id)
            ->leftJoin('inventory_setup_order_lots', 'inventory_setup_order_lot_color_sizes.lot_id', '=', 'inventory_setup_order_lots.id')
            ->leftJoin('inventory_setup_size', 'inventory_setup_order_lot_color_sizes.size_id', '=', 'inventory_setup_size.id')    
            ->leftJoin('inventory_setup_colors', 'inventory_setup_order_lot_color_sizes.color_id', '=', 'inventory_setup_colors.id')
            ->select('inventory_setup_order_lot_color_sizes.*', 'inventory_setup_order_lots.lot_no', 'inventory_setup_size.size_name', 'inventory_setup_colors.color_name')
            ->get();
            return view('ordermanagement::database.basicorders.show', compact('basicorder', 'basicorders', 'buyers', 'organizations', 'product_categories', 'merchandisers', 'fabric_types', 'compositions', 'fabric_treatments', 'yarn_counts', 'yarn_categories', 'ListOfOrdersUniqueBuyer', 'ListOfOrders', 'tab', 'lots', 'colors', 'sizes', 'sizeGroups', 'lotColorsSizes'));
        }
    }
