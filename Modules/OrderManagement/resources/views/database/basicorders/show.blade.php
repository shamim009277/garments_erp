    ...
    <div class="row">
        ...
        <div class="col-12 mb-3">
            <div class="d-flex flex-column flex-md-row align-items-center justify-content-between">
                ...
                @if (1)
                    <!-- Back Button -->
                    <a href="{{ route('ordermanagement.database.basicorders.index') }}"
                                ...
                                <div class="nav-custom-content">
                                    <ul class="nav-custom">
                                        @foreach ($buyerOrders as $order)
                                            <li class="nav-custom-item">
                                                <a href="{{ route('ordermanagement.database.basicorders.show', $order->id) }}?tab=1">
            ...
            <div class="card alert-primary alert-top-border">
                <div class="card-body px-0 py-0" style="min-height: 500px;">
                    <ul class="nav nav-tabs nav-tabs-custom" role="tablist" style="background-color: #5559ca; color: white;border-radius: 0px !important;">
                        <li class="nav-item">
                            <a href="{{ route('ordermanagement.database.basicorders.show', $basicorder->id) }}?tab=1" class="nav-link border-none {{ $tab == 1 ? 'active' : '' }}" title="Basic" role="tab" style="hover: white !important;">
                        ...
                        <li class="nav-item">
                            <a href="{{ route('ordermanagement.database.basicorders.show', $basicorder->id) }}?tab=2" class="nav-link border-none {{ $tab == 2 ? 'active' : '' }}" title="Lot/Ship Info" role="tab">
                                <span class="d-block d-sm-none"><i class="fa fa-credit-card"></i></span>
                                <span class="d-none d-sm-block">Lot/Ship Info</span>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="{{ route('ordermanagement.database.basicorders.show', $basicorder->id) }}?tab=3" class="nav-link border-none {{ $tab == 3 ? 'active' : '' }}" title="Color and Size Info" role="tab">
                                <span class="d-block d-sm-none"><i class="fa fa-credit-card"></i></span>
                    ...
                    <div class="tab-content text-muted">
                        ...
                        <div class="tab-pane {{ $tab == 1 ? 'active' : '' }}" id="basic" role="tabpanel">
                            @include('ordermanagement::database.basicorders.tab1')
                        </div>
                        @endif
                        @if($tab == 2)
                        <div class="tab-pane {{ $tab == 2 ? 'active' : '' }}" id="color" role="tabpanel">
                            @include('ordermanagement::database.basicorders.tab2')
                        </div>
                        @endif
                        @if($tab == 3)
                        <div class="tab-pane {{ $tab == 3 ? 'active' : '' }}" id="lot" role="tabpanel">
                            @include('ordermanagement::database.basicorders.tab3')
                        </div>
                        @endif
                    </div>
                </div>
