    ...
    <div class="row">
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <form id="moduleForm"
                                                    action="{{ route('ordermanagement.setup.items.update', $item->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
        ...
        <div class="col-md-4">
            <div class="card alert-info alert-top-border">
                ...
                <div class="card-body">
                    <form id="moduleForm" action="{{ route('ordermanagement.setup.items.store') }}" method="POST">
                        @csrf
    ...
    <script>
        $(document).ready(function() {
            $('.item-toggle').on('change', function() {
                ...
                $.ajax({
                    url: '{{ route('ordermanagement.setup.items.toggle') }}',
                    type: 'POST',
                    data: {
                        id: id,
                        status: status,
        ...
        $(document).on('click', '.delete-item', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route('ordermanagement.setup.items.delete') }}',
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            id: itemId
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('#goods_category_id').addEventListener('change', function() {
                ...
                fetch(`{{ route('ordermanagement.setup.items.getSubcategories') }}?category_id=${categoryId}`, {
                        headers: {
            ...
            $(document).ready(function() {
                function loadSubcategories(categoryId, selectedId = null) {
                    ...
                    $.ajax({
                        url: '{{ route('ordermanagement.setup.items.getSubcategories') }}',
                        type: 'GET',
                        data: {
