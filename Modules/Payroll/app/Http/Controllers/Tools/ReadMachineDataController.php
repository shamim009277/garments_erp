...
class ReadMachineDataController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Auto increment adjust only once
            $lastid = ReadMachineData::orderBy('id', 'DESC')->first();
            $lastid = $lastid ? $lastid->id + 1 : 1;
            DB::update("ALTER TABLE payroll_tools_read_machinedata AUTO_INCREMENT = " . $lastid . ";");
            ...
            foreach ($files as $file) {
                ...
                foreach ($chunks as $chunk) {
                    ...
                    foreach ($chunk as $line) {
                        ...
                        // Carbon datetime convert
                        $attendanceDate = Carbon::createFromFormat('YmdHis', $date . $time)
                            ->format('Y-m-d H:i:s');
            ...
            return redirect()->back()->with('success', 'Machine Data Read Successfully (Multiple File Processed)');
        } catch (\Throwable $th) {
            return redirect()->back()->with('error', $th->getMessage());
        }
    }
