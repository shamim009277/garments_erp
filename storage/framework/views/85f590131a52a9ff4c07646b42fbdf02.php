        ...
        $(document).ready(function() {
            ...
            $("#done-btn").click(function() {
                ...
                if(validateItem()){
                    ...
                    $.ajax({
                        type: 'PUT',
                        url: "<?php echo e(url('inventory/database/reqapproval/multiplestatus')); ?>/"+req_id,
                        data: {
                            is_confirmed: 1,
            ...
            function updateItem(id){
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        console.log(data);
                        $('#for_qty_'+id).val(Number(data.data.app_qty).toFixed(2));
                        $('#total_value_'+id).text(Number(data.data.total_value).toFixed(2));
                        $('#for_qty_'+id).removeClass('is-valid');
                    ...
                    success: function(data) {
                        ...
                        $.each(detailsData, function(key, value) {
                            ...
                            if(value.is_rejected == 1 && value.rejected_stage == 1){
                                ...
                                let appQtyClass = '';
                                if(value.app_qty <= 0 || value.app_qty == null || value.app_qty == undefined ){
                                    appQtyClass = 'is-invalid';
                                }
                                if(mainData.is_confirmed == 1){
                                ...
                                <tr>
                                    ...
                                    <td width="10%">
                                        <input type="text" class="form-control form-control-sm for_qty ${appQtyClass}" data-price="${value.aprx_priced}" data-id="${value.id}" id="for_qty_${value.id}" value="${value.app_qty?value.app_qty:0}" ${(mainData.is_confirmed == 1 || value.is_rejected == 1) ? 'disabled' : ''}>
                                    </td>
            ...
            function searchRequisition(match, search) {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "<?php echo e(route('inventory.database.reqapproval.search')); ?>",
                    data: {
                        search: search,
                        match: match,
                        _token: "<?php echo e(csrf_token()); ?>"
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH H:\laragon\www\garments_erp\Modules\Inventory\resources\views\database\reqapproval\index.blade.php ENDPATH**/ ?>