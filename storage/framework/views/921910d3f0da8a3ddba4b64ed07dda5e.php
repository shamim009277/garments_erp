                                    ...
                                    <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
                                        <div class="modal-dialog modal-xl">
                                            <div class="modal-content">
                                                ...
                                                <form id="addFormMain" action="<?php echo e(route('inventory.database.purrequisitionmains.store')); ?>" method="POST">
...
<?php endif; ?>
                                                                            <?php if (isset($component)) { $__componentOriginal18c21970322f9e5c938bc954620c12bb = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal18c21970322f9e5c938bc954620c12bb = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.text-input','data' => ['id' => 'required_by_id','name' => 'required_by_id','class' => 'form-control w-100','type' => 'text','readonly' => true,'value' => Auth::user()->id,'hidden' => true]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('text-input'); ?>
<?php if ($component->shouldRender()): ?>
<?php $__env->startComponent($component->resolveView(), $component->data()); ?>
<?php if (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag): ?>
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['id' => 'required_by_id','name' => 'required_by_id','class' => 'form-control w-100','type' => 'text','readonly' => true,'value' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(Auth::user()->id),'hidden' => true]); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
<?php if (isset($__attributesOriginal18c21970322f9e5c938bc954620c12bb)): ?>
<?php $attributes = $__attributesOriginal18c21970322f9e5c938bc954620c12bb; ?>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#done-btn").click(function() {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "<?php echo e(url('inventory/database/purrequisitionmains/multiplestatus')); ?>/"+req_id,
                    data: {
                        is_done: 1,
                        ...
                        $.each(detailsData, function(key, value) {
                            ...
                            if(mainData.is_done == 1){
                                ...
                                element = `<button type="button" data-id="${value.id}" class="btn btn-info btn-sm p-1 update-item">Update</button>
                                        <button type="button" data-id="${value.id}" class="btn btn-danger btn-sm p-1 remove-item">Remove</button>`;
                            }
                                $('#item-row').append(`
                                <tr id="item-row-${value.id}">
                                    ...
                                    <td>${unit.name}</td>
                                    <td width="10%">
                                        <select name="unit_id" class="form-control form-control-sm" id="unit_id_${value.id}" ${mainData.is_done == 1 ? 'disabled' : ''}>
                                            <option value="">Select Unit</option>
                                            ${pur_units.map(x => `<option value="${x.id}" ${x.id == value.pur_unit_id ? 'selected' : ''}>${x.name}</option>`).join('')}
                                        </select>
                                    </td>
                                    <td width="10%"><input type="text" class="form-control form-control-sm" id="req_qty_${value.id}" value="${value.req_qty?value.req_qty:0}" ${mainData.is_done == 1 ? 'disabled' : ''}></td>
                                    <td>
                                    ${element}
                                    </td>
                                </tr>
            ...
            function searchRequisition(match, search) {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "<?php echo e(route('inventory.database.purrequisitionmains.search')); ?>",
                    data: {
                        search: search,
                        match: match,
                        _token: "<?php echo e(csrf_token()); ?>"
            ...
            $('#search_data').on('select2:select', function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: "<?php echo e(route('inventory.database.purrequisitiondetails.store')); ?>",
                    data: {
                        item_id: itemId,
                        req_no: req_no,
                        _token: "<?php echo e(csrf_token()); ?>"
                    ...
                    success: function(data) {
                        ...
                        $('#item-row').append(`
                         <tr class="p-0" id="item-row-${data.details.id}">
                            ...
                            <td width="10%">
                                <select name="unit_id" class="form-control form-control-sm" id="unit_id_${data.details.id}">
                                    <option value="">Select Unit</option>
                                    ${data.units.map(unit => `<option value="${unit.id}">${unit.name}</option>`).join('')}
                                </select>
                            </td>
                            <td width="10%"><input type="text" class="form-control form-control-sm" id="req_qty_${data.details.id}" value="0"></td>

                            <td width="10%">
                                <button type="button" data-id="${data.details.id}" class="btn btn-info btn-sm p-1 update-item">Update</button>
                                <button type="button" data-id="${data.details.id}" class="btn btn-danger btn-sm p-1 remove-item">Remove</button>
                            </td>
                        </tr>
                        `);
                        $('.update-item').off('click').on('click', function() {
            ...
            $('#addFormMain').submit(function(e) {
                ...
                $.ajax({
                    url: '<?php echo e(route('inventory.database.purrequisitionmains.store')); ?>',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
