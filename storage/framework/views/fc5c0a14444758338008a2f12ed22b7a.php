    ...
    <script>
        $(document).ready(function() {
            ...
            $("#done-btn").click(function() {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "<?php echo e(url('inventory/database/reqforwarding/multiplestatus')); ?>/"+req_id,
                    data: {
                        is_forward: 1,
            ...
            function updateItem(id){
                ...
                $.ajax({
                    ...
                    data: {
                        for_qty: forwardQty,
                        forward_date: "<?php echo e($today_date); ?>",
                        forward_by: "<?php echo e(Auth::user()->id); ?>",
                        _token: "<?php echo e(csrf_token()); ?>"
                    },
                    success: function(data) {
                        console.log(data);
                        ...
                        $.each(detailsData, function(key, value) {
                            ...
                            $('#item-row').append(`
                                <tr>
                                    ...
                                    <td width="10%"><input type="text" class="form-control form-control-sm" id="forward_qty_${value.id}" name="forward_qty[]" value="${value.for_qty?value.for_qty:(value.req_qty?value.req_qty:0)}" ${mainData.is_forward == 1 || mainData.is_rejected == 1 ? 'disabled' : ''}></td>
                                    <td>
                                    ${element}
                                    </td>
                                </tr>
                            `);
                            // if(detailsData.every( v => v.is_rejected == 1)){
                            //     $('#done-btn').addClass('d-none');
                            // }else{
                            //     $('#done-btn').removeClass('d-none');
                            // }
            ...
            function searchRequisition(match, search) {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "<?php echo e(route('inventory.database.reqforwarding.search')); ?>",
                    data: {
                        search: search,
                        match: match,
                        _token: "<?php echo e(csrf_token()); ?>"
